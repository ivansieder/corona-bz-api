...
async function getData() {
  try {
    ...
    const sevenDaysAveragePositiveTested = positiveTested.map((_, index) => {
      if (index < 6) {
        return null;
      }
    ...
    const sevenDaysAverageNumberTestedPeople = numberTestedPeople.map((_, index) => {
      if (index < 6) {
        return null;
      }

      return numberTestedPeople.slice(index - 6, index + 1).reduce((previousValue, currentValue) => previousValue + currentValue, 0) / 7;
    });
